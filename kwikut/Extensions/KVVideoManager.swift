...
class KVVideoManager: NSObject {
    ...
    private func doMerge(arrayVideos:[AVAsset], animation:Bool, completion:@escaping Completion) -> Void {
        ...
        var samplesize = CGSize.init(width: 1920.0, height:1080.0)
...
extension KVVideoManager {
    ...
    fileprivate func videoCompositionInstructionForTrack(track: AVCompositionTrack, asset: AVAsset, standardSize:CGSize, atTime: CMTime) -> AVMutableVideoCompositionLayerInstruction {
        ...
        if assetInfo.isPortrait {
            let minus = aspectFillRatio.truncatingRemainder(dividingBy: 1)
            let scaleFactor = CGAffineTransform(scaleX: aspectFillRatio-minus, y: aspectFillRatio-minus)
            
            let posX = standardSize.width/2 - (assetTrack.naturalSize.height * aspectFillRatio)/2
            let posY = standardSize.height/2 - (assetTrack.naturalSize.width * aspectFillRatio)/2
            let moveFactor = CGAffineTransform(translationX: posX-posX, y: posY-posY)
            
            print("scale factor when portrait is true: \(scaleFactor)")
            print("move factor when portrait is true: \(moveFactor)")
            ...
            print("scale factor when portrait is false: \(scaleFactor)")
            print("move factor when portrait is false: \(moveFactor)")
            
            var concat = assetTrack.preferredTransform.concatenating(scaleFactor).concatenating(moveFactor)
            
//            if assetInfo.orientation == .down {
//                print("if orientation is down")
//                let fixUpsideDown = CGAffineTransform(rotationAngle: CGFloat(Double.pi/2))
